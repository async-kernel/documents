...
2. 利用Rust语言的异步无栈协程机制，优化操作系统内核的并发性能；（rCore和zCore的实现，在内核中支持[异步]([https://github.com/rcore-os/executor](https://github.com/rcore-os/executor?fileGuid=OVkmSOUfDpIX2ltk))是现成的；需要做的事，针对异步机制对现有的内核模块进行优化(贾：文件系统中的read和write的系统调用接口已完成改造，还需要对内核中的文件系统模块进行改造；网络模块：在qemu中用virtio；需要询问K210上的无线模块状态；串口模块)；陈渝老师可以支持SiFive的有网络的板子）
...
4. 每个进程应该有自己的中断处理例程（所以xtvec不够用了）
5. 用户态中断的编号同时包括源（用页表基址satp、栈底地址之类的算）、中断类型和目的进程的信息（kill哪个）（硬件哈希）
* 程序运行在用户态上，mstatus.{m,s,u}ie全局中断使能开启、uie.ueie外部中断使能开启
* 按键，在uip.ueip的外部中断中写入1。此时开始处理外部中断（按键）todo全部进程响应（温柔地劝桌面上所有应用结束）
* 如果mideleg、sideleg的对应位都是1，那么我们可以在用户态处理这个中断（这是N扩展的内容）。当我们不跳转到utvec（如果是缺省的话，可以），如果可以，我们直接跳转到专门处理这个中断的程序地址，方法如下
* 每个按键应该有个编号（是这样么？就是区分按的是a还是b的类似东西），用这个查用户态中断专用的Cache（Cache中存放已经批准过的中断源和处理例程地址） 假设编号有4位，用后两位作为地址，在已批准中断Cache里读取内容（应该是这个周期就能读出来，否则中断要过几个周期才能跳转到中断处理例程）
* 取出来的内容中有tag，用tag和编号前两位比较，如果相同则命中，跳到内容中的地址--~~（todo：地址应该有三个么？对应M、S、U态的地址。时钟中断可能所有态都要用到）~~---
应该只记录一个地址，因为其他态的已经处理掉了

-----

##### 初始化的方式：

* 如果不命中，像普通的中断一样处理：跳到stvec，特权级变为S
在位于stvec的S态中断处理程序中决定要跳转到哪（根据中断类型、按键的编号）继续处理，跳转前把要跳转的地址和该中断源的信息（应该是编号。类型也记录么？）写入Cache（方法是用csrw+标准中留白的CSR寄存器地址）。 也就是说，即使这个扩展尽量避免使用软件，在一开始的时候仍然要依赖软件
>为什么采用类似Cache的结构（随机寻址，判断读到的是不是想要的），而不是类似列表的结构（从头匹配到尾，找有没有想要的）？明明后者更像转发表，而且不会碰撞

因为我觉得每次中断来临了，都要遍历一整个表需要的周期数太大了。用户态中断的优点应该是加快了处理速度，用列表挨个匹配就慢了

>这个Cache是软件维护的么？

没有软件它用不起来（最开始要用软件写入已经批准的中断源和处理程序）

但是如果Cache满了，换出时应该是硬件自己完成，软件并不知道

事实上在软件最开始写入时，CSR地址应该只有四个：中断源编号、{M,S,U}态的处理例程地址（如果是0代表不用到这个态处理）。而不是完整的Cache那样分行之类的。对写入来说，只是往四个寄存器写入值

时钟中断如何同时传给三个态处理

之前说到Cache应该记录三个地址，但跳转到M态的地址处理完后，下次怎么跳转到S、U态？这应该要加新的寄存器，可能是真正的可以读取的CSR寄存器，也可能像特权级那样不暴露给外面的寄存器。具体怎样实现还没想好

valid

我可以给板子上的每一个开关编号，但是现实生活中外部中断有编号么？它们是拿啥区分的？

我不知道todo

级联：得知是外部中断，去查编号（中断控制器）
...
[no-std-async-experiments-2](https://github.com/japaric/no-std-async-experiments-2?fileGuid=OVkmSOUfDpIX2ltk)

[no_std async/await - soon on stable](https://ferrous-systems.com/blog/stable-async-on-embedded/?fileGuid=OVkmSOUfDpIX2ltk)
...
1. 基于事件循环机制的异步应用，模型为每个用户线程为一个大事件循环，在循环开头通过 syscall 检查哪些请求已经完成，随即对它们进行处理，处理过程中可能提交新的请求，然后进入下一个循环。目前已有的例子是，Nginx 和 Redis 都将事件机制封装成统一的接口，并提供基于 select/poll/epoll/kqueue/iocp/io_uring 其中的某几种系统调用的实现。如 Redis 的[ae_<event_syscall>.c](https://github.com/openanolis/redis/tree/feat/io_uring/src?fileGuid=OVkmSOUfDpIX2ltk)以及 Nginx 的[ngx_<event_syscall>_module.c](https://github.com/openanolis/nginx/tree/feat/io_uring/src/event/modules?fileGuid=OVkmSOUfDpIX2ltk)。tokio 的底层 mio 应该也是基于 epoll 实现的，但目前并没有深入调研。也就是说，无论应用的代码看起来是什么样子，这种应用的核心要素在于事件循环、addEvent 和 poll。
...
2. 吴一凡：改[异步方案文档](https://shimo.im/docs/473QyY5re6tvXb3w?fileGuid=OVkmSOUfDpIX2ltk)；
...
* io_uring 文档：[https://kernel.dk/io_uring.pdf](https://kernel.dk/io_uring.pdf?fileGuid=OVkmSOUfDpIX2ltk)
* io_uring 相关 syscall 与用户库：[https://openanolis.org/en/sig/high-perf-storage/basic/](https://openanolis.org/en/sig/high-perf-storage/basic/?fileGuid=OVkmSOUfDpIX2ltk)
* io_uring 测试框架：[https://github.com/OpenAnolis/perf-test-for-io_uring](https://github.com/OpenAnolis/perf-test-for-io_uring?fileGuid=OVkmSOUfDpIX2ltk)[https://github.com/OpenAnolis/io_uring-echo-server](https://github.com/OpenAnolis/io_uring-echo-server?fileGuid=OVkmSOUfDpIX2ltk)
* Redis 的 io_uring 实现：[https://github.com/openanolis/redis/tree/feat/io_uring](https://github.com/openanolis/redis/tree/feat/io_uring?fileGuid=OVkmSOUfDpIX2ltk)
* Nginx 的 io_uring 实现：[https://github.com/openanolis/nginx/tree/feat/io_uring](https://github.com/openanolis/nginx/tree/feat/io_uring?fileGuid=OVkmSOUfDpIX2ltk)
* AIO 的新归宿：io_uring：[https://zhuanlan.zhihu.com/p/62682475](https://zhuanlan.zhihu.com/p/62682475?fileGuid=OVkmSOUfDpIX2ltk)
* 基于 io_uring 和绑核的 Rust 异步运行时：[https://github.com/DataDog/scipio](https://github.com/DataDog/scipio?fileGuid=OVkmSOUfDpIX2ltk)
* RSoC: improving drivers and kernel (largely io_uring)[https://www.redox-os.org/news/io_uring-0/](https://www.redox-os.org/news/io_uring-0/?fileGuid=OVkmSOUfDpIX2ltk)把0换为 1，2，3，4 可以看到进一步的系列文章
## **async runtime**

* 金枪鱼之夜：Build an Async Runtime for Rust from Scratch[https://tuna.moe/event/2020/rust-async-runtime/](https://tuna.moe/event/2020/rust-async-runtime/?fileGuid=OVkmSOUfDpIX2ltk)
* async collections in github[https://github.com/stjepang](https://github.com/stjepang?fileGuid=OVkmSOUfDpIX2ltk)
## **多核支持**

* [[第三版 tutorial 多核支持文档]](https://github.com/wyfcyx/osnotes/blob/master/book/v3/K210%25E7%25A7%25BB%25E6%25A4%258D%25E4%25B8%258E%25E5%25A4%259A%25E6%25A0%25B8%25E6%2594%25AF%25E6%258C%2581%25E6%258A%25A5%25E5%2591%258A.md#%25E5%25A4%259A%25E6%25A0%25B8%25E6%2594%25AF%25E6%258C%2581?fileGuid=OVkmSOUfDpIX2ltk),[[代码仓库]](https://github.com/wyfcyx/rCore-Tutorial/tree/multicore?fileGuid=OVkmSOUfDpIX2ltk)
